<template>
  <v-layout class="content" row wrap>
    <v-flex sm12 offset-md3 md6>
      <v-card>
        <v-toolbar dark :class="theme">
          <v-toolbar-title class="white--text">{{
            name[$i18n.locale] || "New Election"
          }}</v-toolbar-title>
        </v-toolbar>
        <v-container fluid>
          <v-form v-model="valid" v-on:submit="submitHandler">
            <v-layout row wrap>
              ...
              <v-flex class="form-field" xs12 md6>
                <v-menu
                  v-model="menu2"
              ...
              <v-flex class="form-field" xs12>
                <upload-button
                  prepend-icon="mdi-filter-variant"
                  title="Voters"
                  :selectedCallback="parseVoterList"
                >
                </upload-button>
              </v-flex>
              <v-flex class="form-field" md6 xs12>
                <v-select
                  label="Department"
                  :items="departments"
                  v-model="theme"
                ...
                >
                  <template slot="item" slot-scope="data">
                    <v-avatar :size="24" :class="data.item.class"> </v-avatar>
                    <v-list-item-content>
                      <v-list-item-title>{{
                        data.item.name
                      }}</v-list-item-title>
              ...
              <v-flex class="form-field" md4 xs12>
                <v-text-field
                  label="Contact Title"
              ...
              <v-flex xs12 class="form-field-submit text-center">
                <v-btn
                  type="submit"
                  :disabled="
                    !valid ||
                    this.name[$i18n.locale] === null ||
                    this.subtitle[$i18n.locale] === null ||
                    submitted ||
                    voterScipers.length === 0
                  "
                  color="primary"
                >
                  Create Election
  ...
  methods: {
      ...
      fr.onload = (event) => {
        ...
        for (let i = 0; i < scipersStrArr.length; i++) {
          if (!/^\d{6}$/.test(scipersStrArr[i])) {
            // show snackbar
            console.error(
              new Error(`Invalid sciper ${scipersStrArr[i]} at line ${i}`)
            );
            return;
          }
        }
    ...
    validateName(name) {
      ...
      return (
        name === null ||
        !!this.name[locale] ||
        "Name field in current locale is required"
      );
    },
    validateSubtitle(subtitle) {
      ...
      return (
        subtitle === null ||
        !!this.subtitle[locale] ||
        "Subtitle field is required for the current locale"
      );
    },
    validateMoreInfo(mi) {
    ...
    submitHandler(e) {
      ...
      for (let i = 0; i < langs.length; i++) {
        ...
        subtitle[lang] =
          this.subtitle[lang] === null ? "" : this.subtitle[lang];
        moreInfo[lang] =
          this.moreInfo[lang] === null ? "" : this.moreInfo[lang];
      }
      if (this.start === this.end) {
        ...
        console.log(
          "start/end same, bumping end by one day",
          this.start,
          this.end
        );
      }
